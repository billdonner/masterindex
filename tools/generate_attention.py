...
LANE_ORDER = {"Shipping": 0, "Needs Attention": 1, "Due Soon": 2, "Recent Activity": 3}
...
def portfolio_now(policy: dict) -> datetime:
    """Current time in the portfolio's own timezone.

    Anchoring this to a declared zone rather than to the host keeps a UTC CI
    runner and the owner's Mac from disagreeing about what day it is, which
    would otherwise shift every "due in N days" by one for part of each day.
    """
    name = (policy.get("timezone") or {}).get("value")
    if name:
        try:
...
def commitment_action(commitment: dict, blockers: list[str], blocker_note: str) -> str:
    """What to actually do next, given ordering and external gates."""
    after = commitment.get("afterCommitmentIds") or []
    parts: list[str] = []
    if after:
        parts.append(f"Starts after {', '.join(after)} is in.")
    if blockers:
        parts.append(f"Resolve the blockers, then ship.{blocker_note}")
    else:
        parts.append("Ship it, then set this commitment's status to 'done' in current/commitments.json.")
    if commitment.get("externalGates"):
        parts.append("The date follows the approval; do not add one until it lands.")
    return " ".join(parts)


def commitment_items(commitments: dict, index: dict, policy: dict, now: datetime) -> list[dict]:
    """Owner-stated release commitments, prioritised by how close the date is.

    A commitment is intent, so it never silently disappears: a passed date
    becomes an overdue card rather than vanishing. Only an explicit status of
    done or dropped clears one.
    """
    rules = policy.get("commitments", {})
    imminent = rules.get("imminentDays", 3)
    soon = rules.get("soonDays", 10)
    entities_by_id = {entity["id"]: entity for entity in index["entities"]}
    today = now.date()

    items: list[dict] = []
    for commitment in commitments.get("commitments", []):
        if commitment.get("status") in ("done", "dropped"):
            continue

        target = commitment.get("targetDate")
        target_date = parse_moment(target).date() if target else None
        blockers = commitment.get("blockedByGapIds") or []
        blocker_note = f" Blocked by: {', '.join(blockers)}." if blockers else ""

        position = commitment.get("queuePosition")
        if target_date is None and position:
            # Ordered rather than dated: timing depends on external approval, so
            # the queue position IS the commitment. Not missing data.
            timing = f"{ordinal(position)} in the release queue"
            priority = {1: "high", 2: "medium"}.get(position, "low")
            body = f"Stated as \"{commitment['statedAs']}\"."
            if commitment.get("preferred"):
                body = f"Preferred for the first slot. {body}"
        elif target_date is None:
            priority = "high" if commitment.get("urgent") else "medium"
            timing = "No date given"
            body = f"Stated as \"{commitment['statedAs']}\"."
        else:
            days = (target_date - today).days
            if days < 0:
                priority, timing = "high", f"{abs(days)} day(s) overdue"
            elif days <= imminent:
                priority, timing = "high", f"due in {days} day(s)"
            elif days <= soon:
                priority, timing = "medium", f"due in {days} day(s)"
            else:
                priority, timing = "low", f"due in {days} day(s)"
            body = (
                f"Target {target_date.isoformat()} ({timing}), stated as "
                f"\"{commitment['statedAs']}\" with {commitment.get('precision', 'unknown')} precision."
            )

        if blockers and priority == "low":
            # A dated commitment with an unresolved blocker is not a low-priority item,
            # however far away the date is - the blocker has to clear first.
            priority = "medium"

        gates = commitment.get("externalGates") or []
        if gates:
            body = f"{body} Waiting on: {', '.join(gates)}."
        queue_note = commitment.get("queueNote")
        if queue_note:
            body = f"{body} {queue_note}"

        note = commitment.get("note")
        if note:
            body = f"{body} {note}"

        entity_ids = [e for e in commitment.get("entityIds", []) if e in entities_by_id]
        items.append(
            card(
                item_id=f"commitment-{commitment['id']}",
                kind="commitment",
                title=f"{commitment['title']} — {timing}",
                body=body,
                action=commitment_action(commitment, blockers, blocker_note),
                evidence=f"current/commitments.json id={commitment['id']} statedOn={commitments.get('statedOn')}",
                priority=priority,
                lane="Shipping",
                entity_ids=entity_ids,
                sort_date=target,
                queue_position=position,
                tags=["commitment"] + (["urgent"] if commitment.get("urgent") else []),
            )
        )
...
def build(root: Path, mark_reviewed: bool) -> dict:
    ...
    commitments_path = root / "current" / "commitments.json"
    commitments = read_json(commitments_path) if commitments_path.exists() else {"commitments": []}
    state_path = root / "current" / "attention-state.json"
    state = read_json(state_path)
    now = portfolio_now(policy)
    ...
    items = [
        *commitment_items(commitments, index, policy, now),
        *gap_items,
        *link_items,
        *task_items(index, tasks, state, policy, now),
        *change_items(index, state, policy, now),
    ]
    ...
    return {
        ...
        "sourceFiles": {
            ...
            "commitments": "current/commitments.json",
        ...
        "lanes": ["Shipping", "Needs Attention", "Due Soon", "Recent Activity"],
        "items": items,
