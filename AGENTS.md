...
Generated by `tools/generate_attention.py` from five inputs:

- `current/index.json` — open gaps and entity link coverage
- `tasks/index.json` — active recurring tasks
- `current/commitments.json` — authoritative owner-stated release commitments
  and target dates; intent rather than observed fact, which is why it is kept
  out of `current/index.json`
...
Record a release date as a commitment in `current/commitments.json`, never as
a fact in `current/index.json`. Preserve the owner's original wording in
`statedAs`, and mark a shipped commitment `done` rather than deleting it.

When a release's timing depends on an external approval, give it a
`queuePosition` and leave `targetDate` null rather than inventing a date. A
null `targetDate` beside a `queuePosition` is a deliberate ordering, not a gap
to fill in.
