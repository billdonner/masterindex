...
- **Collective Engine is deployed but is not live.** `bd-collective-engine` runs on Fly at machine version 20, latest deploy 2026-08-21T01:41Z, with its own `bd-collective-db` Postgres 18.1 cluster created 2026-08-19T02:57Z. Its entity kind moved from `architecture-proposal` to `backend`. It stays `active-prototype` rather than `live` on the index's own terms: the architecture proposal names `https://collective.1041soft.com` the Release 1 canonical URL and calls the `fly.dev` hostname an infrastructure alias, and `collective.1041soft.com` is **NXDOMAIN** — the record was never created, while the apex `1041soft.com` resolves normally to GitHub Pages. So no member can reach the system where Release 1 says it lives, and the reason is that the address was never set up rather than that a host is down; an absent DNS record is durable evidence where a failed probe could be transient. The architecture freeze is still unapproved and no product traffic is served. Do not source status from `~/collective-engine/README.md` line 6, which still claims the service is not deployed and no production database exists; that line is known false.
- **Pickleball Collective has two active local sources.** Collective Engine is the deployed multi-tenant scheduler; Collective Comms is the approval-gated dedicated-Mac messaging utility. Neither is an App Store product. Collective Engine emits file-based, RFC 8785-canonical handoff payloads with verification tooling, exposes court-seat capacity and attendance limit separately so a rotational opening is not shown as a full day, and labels rotations clearly for bookers. Collective Comms defaults one-to-one delivery to SMS, supports explicit overrides, surfaces invalid Messages targets rather than dropping sends silently, and can append an iMessage-only weekly calendar attachment after the text send — SMS is deliberately excluded, and an attachment failure cannot affect the text-delivery record.
- **Oenora Merchant runs as a perpetual simulation.** Its daily loop moved off this Mac's launchd to Fly as `bd-oenora-merchant`, an hourly scheduled machine that sits stopped between ticks.
- **The two Pickleball Collective halves are now one pipeline in the index, not two adjacent records.** `oo-collective-handoff` v1 is recorded as a contract: Collective Engine produces it with `trial --json --out FILE`, Collective Comms consumes it, the format is owned by the producer under ADR-004 and specified in `docs/HANDOFF-FORMAT.md`, and `handoff --verify FILE` checks a payload before it travels. Until 2026-08-23 the index named neither the contract nor the spec, and `collective-comms` still declared a dependency on `collective-engine` — the id retired in the fork merge — so the one edge that mattered pointed at nothing. That was not isolated: **59 dependency references across the index resolved to no entity at all**, because `dependencies` held informal names rather than ids. They are resolved, the deliberate non-entities are declared under `externalDependencies` with six marked `unmapped` (a real dependency whose target has no record, which is an index gap rather than an external system), and `tools/validate-index.py` now fails on an unresolved one.
- **Where a thing runs is recorded for the two cases where it is not a free choice.** `hosts` names `m1-residence` (Apple M1 Pro, always on, verified directly on 2026-08-23: both `com.oopickleball.collective-comms` launch agents installed and loaded) and `m3-travel` (a laptop this index has not observed). Collective Comms is bound to the M1 rather than merely installed there: sending goes through Messages.app and Contacts by AppleScript, and macOS records Contacts and Automation permission against the binary's code identity, which a person must grant at the machine with the screen unlocked. A second Mac therefore cannot take over remotely after the first fails — it has to be prepared beforehand, and only one machine may hold the nightly job because the send ledger and run lock are per-machine. `collective-comms drafts` needs no permission and has no transport at all, which makes it the fallback that works from anywhere. Collective Engine, by contrast, needs no host: `trial` reads `docs/SCHEDULE-LOG.md` and runs the real solver with no database and no network.
- The registry tracks 57 repositories and 59 entities, 36 of which are non-app. The count rose by two when the fork with `origin/main` was resolved on 2026-08-22, bringing in Collective Comms and Oenora Merchant.
...
- **The index fork is resolved.** `current/index.json` was merged on 2026-08-22 after the owner adjudicated the three contested records. A textual merge had to be rejected: git resolved only one line by itself (`generatedAt`) and produced structurally invalid JSON, while silently emitting `app-feedback-inbox` twice, keeping the Collective Engine as two separate entities, and resurrecting the deleted MastPex records. The index was therefore merged by construction, not by line. This branch won every contested record — for all 18 shared entities `origin/main` had touched, this lineage's `lastModified` was at least as recent. `origin/main` contributed the entities `collective-comms` and `oenora-merchant`, the `Pickleball Collective` cluster, and one `deployment-source-recovery` gap. MastPex stays deleted (owner: kaput). `app-feedback-inbox` kept this branch's record, a strict superset. The Collective Engine kept `collective-engine-backend` / `backend` / `active-prototype`, confirmed against live Fly state by the session holding `~/collective-engine`, which also corrected the machine version to 20 and the deploy date to 2026-08-21T01:41Z — both lineages carried the stale 16 and 2026-08-19. A dangling `entityId` reference to `famster-ios-app`, present only in this lineage, was repaired to `famster-app`; `tasks/index.json` entry coverage was reconciled to 59 of 59 entities, dropping the two MastPex keys.
