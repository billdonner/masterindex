...
- workin On is TestFlight-only by owner decision (2026-09-04) and now ships through an Xcode Cloud archive lane (push to `release`), the first BillDonner.com app to do so; its `main` branch carries the MasterIndex board but has not merged with the shipping GitHub-tabs line.
